// harden: true for internal images. Wrap-build: harden.sh on the imported image, flatten to one
// layer, then re-emit the source config (the flatten drops it) and bake the labels in.
...
def call(Map rec) {
    ...
    sh "python3 ${ctx}/wrap.py --from ${pinned} --config ${ctx}/image-config.json --labels ${ctx}/labels.json --out ${ctx}/Dockerfile"
