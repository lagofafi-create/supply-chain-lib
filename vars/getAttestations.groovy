// All attestations attached to an image digest, from the Supply Chain API. Empty list while
// api.enabled is false.
// Response: [ { "predicateType": "https://slsa.dev/provenance/v1", "digest": "sha256:...",
//               "issuer": "...", "createdAt": "..." }, ... ]
def call(String ref, Map opts = [:]) {
    def c = scConfig()
    if (!c.api?.enabled) {
        echo "getAttestations skipped: Supply Chain API not enabled (api.enabled)"
        return []
    }
